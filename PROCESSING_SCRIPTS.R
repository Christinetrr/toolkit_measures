...
wordlist_v2_0 <- read_excel("wordlist_v2.0.xlsx")
...
map_PG <- function(spelling,pronunciation, map_progress=FALSE){
  ...
  map_m <- function(p_m,syll_struc.latest,parsed_syll.latest,letter_str.latest){
    ...
    #don't allow /e/[schwa] --> IA or EA or IE or IA_E if p_wf_prior is actually a /j/ or /i/
    no_e_ia <- FALSE
    no_e_ea <- FALSE
    no_e_ie <- FALSE
    no_e_ia_e <- FALSE
    ...
    if(map_j == TRUE) {
      ...
      if("n" %in% c(stri_sub(letter_str.latest,-1,-1) ,stri_sub(letter_str.latest,-3,-3) ) & p_m_prior=="n" & stri_sub(parsed_syll0,-my.index+1,-my.index+1) !="u"){ #if the letter string up to here ends N and the p_m_prior == "n", then this is probably a rare /nj/ mapping as in lasaGNa or seNor--unless its NJU like NUCLEAR
        flag.nj<<-TRUE
  ...
  map_sf <- function(p_sf,syll_struc.latest,parsed_syll.latest,letter_str.latest){
    ...
    #look at the phoneme in front of the p_sf, to check if its /h/ or /D/ or /T/ or /C/. If it is, then don't allow silent H options if the current p_sf is a vowel
    no_silent_H <- FALSE
    ifelse(p_sf_prior == "h" & p_sf %in% vowels | p_sf_prior == "D" & p_sf %in% vowels | p_sf_prior == "C" & p_sf %in% vowels | p_sf_prior == "T" & p_sf %in% vowels, no_silent_H <- TRUE, no_silent_H <- no_silent_H)
...
map_value <- function(spelling, pronunciation, level, tables, verbose = TRUE) {
  
  if(verbose == TRUE){pb = txtProgressBar(min = 0, max = length(spelling), initial = 0)}
...
pw_spell <- function(pronunciation, level, tables, minimum = 1, param = "pg") {
  ...
  prep_pword <- function(word_index){
    ...
    #get parsed syllables
    hold0 <- as.data.frame(as.vector(sapply(pronunciation[word_index],parse_syllables_PWSPELL))) #hold on to the list of syllables
    hold0 <- hold0[hold0!="",] #this will be able to identify if its a word like /ko ko/ or /ha ha/ that repeats the same syllable twice
    parsed_syll <- matrix(strsplit(hold0,"   ")[[1]])
  ...
  parse_syllables_PWSPELL <- function(pronunciation,word_index) {
  ...
  prep_pword_ONC <- function(mylist2) {
    ...
    for(i in 1:length(pronunciation)){
  ...
  combine_vectors <- function(lists, current = character()) {
    ...
    # Recursive case: for each row in the first matrix, append it to current and call the function recursively
    result <- NULL
    for (row in 1:nrow(lists[[1]])) {
      result <- c(result, combine_vectors(lists[-1], c(current, lists[[1]][row, 1])))
    }
    return(result)
  }
  ...
  check_biphones <- matrix(NA,nrow=length(pronunciation))
  for(i in 1:length(pronunciation)) {
    check_biphones[i] <- grepl(phonemes_to_check, pronunciation[i], ignore.case = FALSE)
  }
  ...
  for(i in 1:length(pronunciation)){
    ...
    #add a third row to start receiving the graphemes, BUT ONLY IF level = "PG"! (its redundant if level = "OR"):
    if(level=="PG") mylist2[[i]] <- rbind(mylist2[[i]],mylist2[[i]][2,]) 
    
    #BELOW IS CRITICAL: default minimum = 1 will use max() to pull out only the most likely spelling
    #if set to FALSE, however, then it will instead pull out ALL non-zero options
    mytemp <- list()
    
    suppressWarnings({
      if(minimum == 1){
        for(j in 1:ncol(mylist2[[i]])) { #now, for each phoneme in that word, PULL OUT THE MAX:  
          mytemp[[j]] <- tables[[param]][tables[[param]]$phoneme==mylist2[[i]][2,j],][,2][which(tables[[param]][tables[[param]]$phoneme==mylist2[[i]][2,j],][,as.numeric(mylist2[[i]][1,j])+2] == max(as.numeric(as.matrix(tables[[param]][tables[[param]]$phoneme==mylist2[[i]][2,j],][,as.numeric(mylist2[[i]][1,j])+2])))),]
        }
      } else { #HERE INSTEAD PULL OUT ALL NON-ZERO OPTIONS  
        for(j in 1:ncol(mylist2[[i]])) { #now, for each phoneme in that word, pull out all the non-zero options:
          mytemp[[j]] <- tables[[param]][tables[[param]]$phoneme==mylist2[[i]][2,j],][,2][which(as.numeric(as.matrix(tables[[param]][tables[[param]]$phoneme==mylist2[[i]][2,j],][,as.numeric(mylist2[[i]][1,j])+2])) > minimum),]
        }
      }
    })
    
    #mytemp is now lists with vectors of each position-specific option, these need to be pasted into words before moving from i to i+1
    mytemp <- unique(combine_vectors(mytemp))
    mymat[[i]] <- mytemp[!grepl("NA|character\\(0\\)", mytemp)]
  }
  
  #at this point, mymat[[i]] contains all the spellings for the default parse; if check_biphones[i] is TRUE, however:
  #before returning mymat, convert the Toolkit phonemes into the single-character format (e.g., "ju" becomes "œ") and repeat all the steps, except c() mymat, don't overwrite it!
  
  if( check_biphones[i] == TRUE) {
    pronunciation[i] <- replace_patterns(pronunciation[i],correspondences)
    mylist1[[i]] <- prep_pword(i)
    mylist2[[i]] <- matrix(strsplit(mylist1[[i]][1,],"")[[1]],ncol=length(strsplit(mylist1[[i]][1,],"")[[1]])) #turn the positions into columns
    mylist2[[i]] <- rbind(mylist2[[i]],matrix(strsplit(mylist1[[i]][2,],"")[[1]],ncol=length(strsplit(mylist1[[i]][2,],"")[[1]]))) #rbind that with the phonemes
    mylist2[[i]] <- reverse_replace_patterns(mylist2[[i]], correspondences_reverse) #convert back to Toolkit format!
    if(level=="PG") mylist2[[i]] <- rbind(mylist2[[i]],mylist2[[i]][2,]) 
    ifelse(level=="OR", mylist2 <- prep_pword_OR(mylist2),
           ifelse(level=="OC", mylist2 <- prep_pword_OC(mylist2),
                  ifelse(level=="ONC", mylist2 <- prep_pword_ONC(mylist2),NA)))
    ...
    mytemp <- unique(combine_vectors(mytemp))
    mytemp <- mytemp[!grepl("NA|character\\(0\\)", mytemp)]
    mymat[[i]] <- unique(c(mymat[[i]],mytemp)) #this will add on the new spellings (with the alternative parsing) onto whatever was generated for the default parsing
  }
  return(mymat)
}


#fourth: map all the words at all grain sizes
all_words_PG <- map_PG(spelling=wordlist_v2_0$spelling,pronunciation = wordlist_v2_0$pronunciation, map_progress = TRUE)
all_words_ONC <- map_ONC(spelling=wordlist_v2_0$spelling,pronunciation = wordlist_v2_0$pronunciation, map_progress = TRUE)
all_words_OC <- map_OC(spelling=wordlist_v2_0$spelling,pronunciation = wordlist_v2_0$pronunciation, map_progress = TRUE)
all_words_OR <- map_OR(spelling=wordlist_v2_0$spelling,pronunciation = wordlist_v2_0$pronunciation, map_progress = TRUE)


#fifth: create frequency-weighted, un-weighted, and non-positional tables, given those mapped words
...
all_tables_ONC_noposition <- make_tables(all_words_ONC, positional = FALSE)
all_tables_OC_noposition <- make_tables(all_words_OC, positional = FALSE)
all_tables_OR_noposition <- make_tables(all_words_OR, positional = FALSE)

all_tables_PG_freq <- make_tables(all_words_PG,weight=wordlist_v2_0$freq)
all_tables_ONC_freq <- make_tables(all_words_ONC,weight=wordlist_v2_0$freq)
all_tables_OC_freq <- make_tables(all_words_OC,weight=wordlist_v2_0$freq)
all_tables_OR_freq <- make_tables(all_words_OR,weight=wordlist_v2_0$freq)

all_tables_PG_freq_noposition <- make_tables(all_words_PG,weight=wordlist_v2_0$freq, positional = FALSE)
all_tables_ONC_freq_noposition <- make_tables(all_words_ONC,weight=wordlist_v2_0$freq, positional = FALSE)
all_tables_OC_freq_noposition <- make_tables(all_words_OC,weight=wordlist_v2_0$freq, positional = FALSE)
all_tables_OR_freq_noposition <- make_tables(all_words_OR,weight=wordlist_v2_0$freq, positional = FALSE)


#sixth: score all the words, given the mappings and tables
scored_words_PG <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "PG", tables = all_tables_PG)
scored_words_ONC <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "ONC", tables = all_tables_ONC)
scored_words_OC <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "OC", tables = all_tables_OC)
scored_words_OR <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "OR", tables = all_tables_OR)

scored_words_PG_noposition <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "PG", tables = all_tables_PG_noposition)
scored_words_ONC_noposition <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "ONC", tables = all_tables_ONC_noposition)
scored_words_OC_noposition <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "OC", tables = all_tables_OC_noposition)
scored_words_OR_noposition <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "OR", tables = all_tables_OR_noposition)

scored_words_PG_freq <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "PG", tables = all_tables_PG_freq)
scored_words_ONC_freq <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "ONC", tables = all_tables_ONC_freq)
scored_words_OC_freq <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "OC", tables = all_tables_OC_freq)
scored_words_OR_freq <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "OR", tables = all_tables_OR_freq)

scored_words_PG_freq_noposition <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "PG", tables = all_tables_PG_freq_noposition)
scored_words_ONC_freq_noposition <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "ONC", tables = all_tables_ONC_freq_noposition)
scored_words_OC_freq_noposition <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "OC", tables = all_tables_OC_freq_noposition)
scored_words_OR_freq_noposition <- map_value(spelling = wordlist_v2_0$spelling, pronunciation = wordlist_v2_0$pronunciation, level = "OR", tables = all_tables_OR_freq_noposition)


#Examples of extracting information

#phonographeme spelling consistency for the word "penguin"
summarize_words(mapped_words = scored_words_PG[which(wordlist_v2_0$spelling=="penguin")], parameter = "PG") #unweighted
summarize_words(mapped_words = scored_words_PG_freq[which(wordlist_v2_0$spelling=="penguin")], parameter = "PG") #frequency-weighted
...
pw_spell("flok","PG",all_tables_PG,minimum=1) #most likely spelling for /flok/ at PG level

pw_spell("flok","ONC",all_tables_ONC,minimum=1) #most likely spelling for /flok/ at ONC level

pw_spell("flok","OR",all_tables_OR,minimum=0) #all spellings for /flok/ at OR level

pw_spell("flok","OC",all_tables_OC,minimum=0) #all spellings for /flok/ at OC level
